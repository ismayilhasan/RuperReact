...
function ColorsForm() {
  ...
  const handleDeleteColor = (id) => {
    ...
    axios.delete(`https://localhost:7216/api/Colors/completelyDelete/${id}`)
      .then(response => {
                ...
                {isSuccess &&
                  colors.map((color) => (
                    <tr key={color.id}>
                      ...
                      <td >
                          <Link className="btn btn-warning me-2" onClick={() => updateColor(color.id)} >Update</Link>
                          <Link onClick={() => handleDeleteColor(color.id)} className="btn btn-danger">Delete</Link>
                      </td>
